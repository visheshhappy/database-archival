 ...
 */
package com.snapdeal.archive.controller;
...
public class ArchivalController {
    ...
    public String archieveVerifyAndDeleteData(HttpServletRequest request, @PathVariable("tableName") final String tableName, @PathVariable("batchSize") String batchSize)
            throws BusinessException {
        SystemLog.logMessage("Table name is  : " + tableName);
        final String criteria = "where " + request.getParameter("whereClause");
        final Long batch = Long.valueOf(batchSize);
        final String strategy = request.getParameter("strategy");
        final String archiveInfoName = request.getParameter("archiveInfoName");
        final ArchivalStrategyType archivalStrategyType = getStrategyType(strategy);
